...
const ucptJSON = JSON.parse(await fse.readFile('.ucpt.config.json', 'utf8'));
...
class BuildTools extends FileHandle {
    constructor() {
        super();
        // 缓存初始数据
        this.cache = {};
        ...
        console.log(ucptJSON);

        this.ucptConfig = ucptJSON;
    ...
    async localBuild() {
        ...
        console.log('app资源包打包完成');

        if(this.ucptConfig.localBuildAfterScript && this.ucptConfig.localBuildAfterScript.script) {

            await this.cmdRun(this.ucptConfig.localBuildAfterScript.script, Path.resolve(this.ucptConfig.localBuildAfterScript.cwd));

            await this.cmdRun(`start explorer ${Path.resolve(this.ucptConfig.localBuildAfterScript.cwd)}`);

            console.log('打包之后的命令完成 nice');
        }
    }
    ...
    async loopWorkTask(index) {
        ...
        if (param) {
            ...
            // 过滤小于打包时间的文件
            await fse.ensureDir('unpackage/release/apk/');
    ...
    cmdRun(script, cwd) {
        ...
        if(cwd) {
            script = `cd ${cwd} && ${script}`;
        }
        const encoding = 'cp936';
        const binaryEncoding = 'binary';
        return new Promise((resolve, reject) => {
            const childProce = childProcess.exec(script, { encoding: binaryEncoding, cwd }, (error, stdout, stderr) => {
                if (error) {
